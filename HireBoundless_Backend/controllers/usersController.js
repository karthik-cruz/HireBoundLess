...
const loginUser = async (req, res) => {
    try {
        ...
        if (!user) {
            ...
            if (!isMatch) {
                return res.status(202).json({ message: 'Incorrect password', success: false });
            }
        }
...
const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        const uniqueRandomNumber = Math.floor(Math.random() * 1000000000000000);
        cb(null, file.originalname.slice(0, -path.extname(file.originalname).length) + "-" + uniqueRandomNumber + path.extname(file.originalname));
    }
});
...
const profileCreate = async (req, res) => {
    upload.single('resume')(req, res, async (err) => {
        ...
        try {
            ...
            // Check if the user has an existing resume and delete the old file if it exists
            if (user.profile.resume) {
                const existingResume = path.join(__dirname, '..', 'uploads', 'resumes', user.profile.resume);

                // Make sure the file exists before trying to delete it
                if (fs.existsSync(existingResume)) {
                    fs.unlinkSync(existingResume); // Delete the old resume file
                }
            }
            ...
            // Update user's profile with the new resume filename
...
const avatarStorage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        const uniqueRandomNumber = Math.floor(Math.random() * 1000000000000000);
        cb(null, 'avatar-' + uniqueRandomNumber + path.extname(file.originalname));
    }
});
    ...
    uploadAvatar.single('avatar')(req, res, async (err) => {
        ...
        try {
            ...
            if (user.profile.avatar) {
                const existingAvatar = path.join(__dirname, '..', 'uploads', 'avatars', user.profile.avatar);

                // Make sure the file exists before trying to delete it
                if (fs.existsSync(existingAvatar)) {
                    fs.unlinkSync(existingAvatar); // Delete the old resume file
                }
            }
